...
export default function PostDetailPage() {
  ...
  useEffect(() => { if (!id) return; contentApi.get<Post>("/posts/" + id).then(p => { setPost(p); setLikeCount(p.likeCount); setLoading(false); }); contentApi.get<{ items: Comment[] }>("/posts/" + id + "/comments").then(d => setComments(d.items)); }, [id]);
  ...
  const submit = async () => { if (!text.trim() || !id) return; try { const r: { id: string } = await contentApi.post("/posts/" + id + "/comments", { content: text }); setComments(prev => [...prev, { id: r.id, content: text, authorName: "我", createdAt: new Date().toISOString() }]); setText(""); } catch {} };
    ...
    <div style={s.container}>
      ...
      <article style={s.article}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 28, paddingBottom: 28, borderBottom: "1px solid var(--border-light)" }}>
          ...
          <div><div style={{ fontSize: 16, fontWeight: 700, color: "var(--text-strong)" }}>{post.authorName}</div><div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 2 }}>{new Date(post.createdAt).toLocaleDateString("zh-CN")} · {post.viewCount} 次阅读</div></div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 32, paddingTop: 28, borderTop: "1px solid var(--border-light)" }}>
          <button onClick={like} style={{ ...s.btn, background: liked ? "var(--red-ghost)" : "var(--bg-hover)", color: liked ? "var(--red)" : "var(--text-soft)", border: liked ? "1px solid var(--red)" : "1px solid var(--border)" }}><svg width="16" height="16" fill={liked ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>{likeCount}</button>
        </div>
      </article>
      <section style={s.article}>
        <h2 style={{ fontSize: 19, fontWeight: 700, marginBottom: 24 }}>评论 {comments.length}</h2>
        {isLoggedIn && (<div style={{ marginBottom: 28 }}><textarea style={s.input} rows={3} value={text} onChange={e => setText(e.target.value)} placeholder="写下你的评论..." onFocus={e => { e.currentTarget.style.borderColor = "var(--ow-orange)"; }} onBlur={e => { e.currentTarget.style.borderColor = "var(--border)"; }} /><div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12 }}><button onClick={submit} disabled={!text.trim()} style={{ ...s.btn, background: text.trim() ? "var(--ow-orange)" : "var(--bg-hover)", color: text.trim() ? "#fff" : "var(--text-muted)", border: "none", boxShadow: text.trim() ? "0 2px 8px rgba(240,100,36,0.3)" : "none" }}>发布评论</button></div></div>)}
        {comments.length === 0 ? (<div style={{ textAlign: "center", padding: 48, color: "var(--text-muted)", fontSize: 15 }}>还没有评论</div>) : (comments.map(c => (<div key={c.id} style={s.comment}><div style={s.avatarSm(c.authorName)}>{c.authorName?.[0]?.toUpperCase()}</div><div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}><span style={{ fontSize: 15, fontWeight: 700, color: "var(--text-strong)" }}>{c.authorName}</span><span style={{ fontSize: 12, color: "var(--text-muted)" }}>{new Date(c.createdAt).toLocaleDateString("zh-CN")}</span></div><p style={{ fontSize: 15, color: "var(--text-body)", lineHeight: 1.7 }}>{c.content}</p></div></div>)))}
      </section>
    </div>
  );
}
