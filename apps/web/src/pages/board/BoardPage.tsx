...
export default function BoardPage() {
  ...
  useEffect(() => { if (isLoggedIn) { socialApi.getNotifications().then((d) => setUnreadNotifs(d.unreadCount)); } }, [isLoggedIn, token]);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3">
          {isLoggedIn ? (
            <>
              ...
              <button onClick={() => { logout(); navigate('/login'); }} className="text-gray-500 text-sm hover:text-red-400">退出</button>
              <Link to="/post/new" className="px-4 py-2 bg-ow-orange text-black font-bold rounded-lg hover:bg-orange-400 text-sm">发帖</Link>
            </>
          ) : (
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        <button onClick={() => setActiveBoard('')} className={'px-4 py-2 rounded-lg text-sm whitespace-nowrap ' + (!activeBoard ? 'bg-ow-blue text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700')}>全部</button>
        {boards.map(b => <button key={b.id} onClick={() => setActiveBoard(b.id)} className={'px-4 py-2 rounded-lg text-sm whitespace-nowrap ' + (activeBoard === b.id ? 'bg-ow-blue text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700')}>{b.name}</button>)}
      </div>
      ...
      {loading ? <div className="text-center text-gray-500 py-20">加载中...</div> : posts.length === 0 ? <div className="text-center text-gray-500 py-20"><p className="text-lg mb-2">还没有帖子</p>{isLoggedIn && <Link to="/post/new" className="text-ow-blue hover:underline">发布第一条帖子</Link>}</div> : (
        <div className="space-y-3">
          {posts.map(post => (
            <div key={post.id} className="p-4 bg-ow-darker rounded-xl hover:bg-gray-800 transition-colors">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                ...
                <button onClick={() => handleLike(post.id)} className="hover:text-red-400">{post.likeCount} 赞</button>
