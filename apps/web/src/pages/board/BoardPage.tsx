...
export default function BoardPage() {
  ...
  useEffect(() => {
    ...
    contentApi.get<{ items: Post[] }>('/posts' + params).then(d => setPosts(d.items)).finally(() => setLoading(false));
  }, [activeBoard]);
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-4">
          {isLoggedIn ? (<><span className="text-gray-400 text-sm">{user?.username}</span><Link to="/post/new" className="px-4 py-2 bg-ow-orange text-black font-bold rounded-lg hover:bg-orange-400 transition-colors text-sm">发帖</Link></>) : (<Link to="/login" className="px-4 py-2 bg-ow-blue rounded-lg text-sm hover:bg-blue-600">登录</Link>)}
        </div>
      </div>
