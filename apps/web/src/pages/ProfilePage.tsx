...
interface Comment { id: string; content: string; postId: string; createdAt: string; }

const s = {
  container: { maxWidth: 1000, margin: '0 auto' } as const,
  header: { background: 'var(--bg-surface)', borderRadius: 18, padding: '36px 40px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--border-light)', marginBottom: 24, display: 'flex', gap: 24, alignItems: 'center' } as const,
  avatar: { width: 72, height: 72, borderRadius: 20, background: 'linear-gradient(135deg, var(--ow-orange), var(--ow-orange-light))', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, fontWeight: 700, flexShrink: 0 } as const,
  section: { background: 'var(--bg-surface)', borderRadius: 18, padding: '28px 36px', boxShadow: 'var(--shadow-card)', border: '1px solid var(--border-light)', marginBottom: 20 } as const,
  tab: (active: boolean) => ({ padding: '8px 20px', borderRadius: 10, fontSize: 14, fontWeight: active ? 600 : 500, cursor: 'pointer', border: 'none', background: active ? 'var(--ow-orange-ghost)' : 'transparent', color: active ? 'var(--ow-orange)' : 'var(--text-soft)', transition: 'all .15s' }),
...
export default function ProfilePage() {
  ...
  const toggleFollow = async () => {
    if (!profile) return;
    try {
      if (following) { await socialApi.unfollowUser(profile.id); setFollowing(false); setFollowerCount(c => c - 1); }
      else { await socialApi.followUser(profile.id); setFollowing(true); setFollowerCount(c => c + 1); }
    } catch {}
  };
      ...
      <div style={s.header}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <h1 style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-strong)' }}>{profile.nickname || profile.username}</h1>
            {profile.nickname && <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>@{profile.username}</span>}
          </div>
          {profile.bio ? (
        ...
        {isOwnProfile ? (
          <Link to="/profile/edit" style={{ padding: '10px 22px', borderRadius: 12, border: '1px solid var(--border)', color: 'var(--text-soft)', textDecoration: 'none', fontSize: 14, fontWeight: 500, background: 'var(--bg-hover)', transition: 'all .15s' }}>
            编辑资料
          </Link>
        ) : (
          <button onClick={toggleFollow} style={{ padding: '10px 22px', borderRadius: 12, border: following ? '1px solid var(--border)' : 'none', background: following ? 'var(--bg-hover)' : 'var(--ow-orange)', color: following ? 'var(--text-soft)' : '#fff', cursor: 'pointer', fontSize: 14, fontWeight: 600, transition: 'all .15s' }}>
            {following ? '已关注' : '关注'}
          </button>
        )}
      </div>
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        <button style={s.tab(tab === 'posts')} onClick={() => setTab('posts')}>帖子 ({posts.length})</button>
        <button style={s.tab(tab === 'comments')} onClick={() => setTab('comments')}>评论 ({comments.length})</button>
      </div>
      <div style={s.section}>
        {tab === 'posts' ? (
          posts.length === 0 ? <div style={{ textAlign: 'center', padding: 48, color: 'var(--text-muted)' }}>暂无帖子</div> : posts.map(post => (
            <div key={post.id} style={s.postItem}>
              <Link to={'/post/' + post.id} style={{ textDecoration: 'none', color: 'inherit' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)', display: 'flex', gap: 14 }}>
                  <span>{post.boardName}</span><span>❤ {post.likeCount}</span><span>💬 {post.commentCount}</span><span>{ago(post.createdAt)}</span>
                </div>
              </Link>
            </div>
          ))
        ) : (
          comments.length === 0 ? <div style={{ textAlign: 'center', padding: 48, color: 'var(--text-muted)' }}>暂无评论</div> : comments.map(c => (
            <div key={c.id} style={s.postItem}>
              <Link to={'/post/' + c.postId} style={{ textDecoration: 'none', color: 'inherit' }}>
                <p style={{ fontSize: 14, color: 'var(--text-body)', lineHeight: 1.6, marginBottom: 4 }}>{c.content.length > 200 ? c.content.slice(0, 200) + '...' : c.content}</p>
