...
export default function AppLayout() {
  ...
  useEffect(() => {
    if (isLoggedIn) socialApi.getNotifications().then(d => setUnread(d.unreadCount));
  }, [isLoggedIn]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-page)" }}>
      <header style={{
        background: "var(--bg-surface)", boxShadow: "var(--shadow-nav)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 28, maxWidth: 1200, margin: "0 auto", width: "100%" }}>
          ...
          <nav style={{ display: "flex", gap: 2, flex: 1 }}>
            <Link to="/boards" style={{ padding: "6px 14px", borderRadius: 8, fontSize: 13, fontWeight: isActive("/boards") ? 600 : 500, color: isActive("/boards") ? "var(--ow-orange)" : "var(--text-soft)", background: isActive("/boards") ? "var(--ow-orange-ghost)" : "transparent", textDecoration: "none", transition: "all .15s" }}>Boards</Link>
          </nav>
