...
export class CommentsController {
  ...
  getComments(@Param('postId') postId: string, @Query() query: ListCommentsQuery): ApiResponse {
    ...
    const items = this.db.prepare(
      "SELECT c.id, c.content, c.parentId, c.replyToId, c.replyToAuthorName, c.likeCount, c.createdAt, COALESCE(u.nickname, u.username) as authorName FROM comments c LEFT JOIN users u ON c.authorId = u.id WHERE c.postId = ? AND c.status = ? ORDER BY c.createdAt ASC LIMIT ? OFFSET ?"
  ...
  createComment(@Param('postId') postId: string, @Body() dto: CreateCommentDto, @Req() req: { user: { userId: string; username: string; nickname: string | null } }): ApiResponse {
    ...
    const id = uuidv4();
    this.db.prepare(
      'INSERT INTO comments (id, postId, authorId, parentId, replyToId, replyToAuthorName, content) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ).run(id, postId, userId, dto.parentId || null, dto.replyToId || null, dto.replyToAuthorName || null, dto.content);
    ...
    return { code: 0, message: '评论成功', data: { id, content: dto.content, parentId: dto.parentId, replyToId: dto.replyToId, replyToAuthorName: dto.replyToAuthorName, authorName, createdAt: new Date().toISOString() } };
  }
}